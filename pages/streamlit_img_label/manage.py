...
class ImageManager:
    ...
    def _load_rects(self):
        """Loads rectangles from the associated JSON file.

        Modified to correctly construct the JSON file path.
        """
        base_name = os.path.splitext(os.path.basename(self._filename))[0]
        json_path = os.path.join(os.path.dirname(self._filename), f"{base_name}.json")

        if os.path.exists(json_path):
            rects_json = read_json(json_path)  # Pass the full path to read_json
            if rects_json:
    ...
    def save_annotation(self):
        """output the json annotation file."""
...
class ImageDirManager:
    ...
    def _get_next_image_helper(self, index):
        while index < len(self._files) - 1:
            ...
            if f"{image_file_name}.json" not in self._annotations_files:
                return index
        return None
